---
title: "Chapter 03. REST API Design"
permalink: /aws-backend/part-16/03-rest-api-design/
...
# Chapter 03. REST API Design
## HTTP 의미와 재시도 안전성을 보존하는 주문 API

> **학습 목표**
>
> - 리소스 중심 URI와 HTTP 메서드 의미를 적용한다.
> - 멱등성 키로 주문과 결제 재시도를 안전하게 처리한다.
> - Cursor 기반 페이지네이션과 표준 오류 응답을 설계한다.
> - OpenAPI와 버전 정책을 계약으로 관리한다.

---

# 요구사항과 실패 시나리오

상품은 `/v1/products/{productId}`로 식별하고 동사를 URI에 넣지 않는다.

주문 생성은 `POST /v1/orders`를 사용하고 성공 시 201과 Location을 반환한다.

조회 성공은 200, 비동기 접수는 202, 본문 없는 삭제 성공은 204를 사용한다.

검증 실패는 400, 인증 부재는 401, 권한 부족은 403, 충돌은 409로 구분한다.

클라이언트는 주문 생성에 `Idempotency-Key`를 보내고 서버는 사용자와 키와 요청 해시를 저장한다.
...
Client
  ...
  v
OrderController -- @Valid --> OrderService
  |                              |
  | 201 / 409                    +--> Aurora transaction
  |                              +--> Idempotency record
  v
Problem response + traceId
OpenAPI contract <- CI compatibility check
```
...
| 기준 | 선택 A | 선택 B | 결정 기준 |
|---|---|---|---|
| 주제 | Offset 방식 | Cursor 방식 | 결정 |
| 페이지 이동 | 임의 페이지 쉬움 | 순차 탐색 중심 | 상품 피드는 Cursor |
| 변경 데이터 | 중복 가능 | 상대적으로 안정 | 고유 정렬 키 필수 |
| 구현 | 단순 | 토큰 관리 | 계약 문서화 |
| 비용 | 깊은 Offset 비쌈 | 인덱스 탐색 | 실행 계획 검증 |
...
1. 상품은 `/v1/products/{productId}`로 식별하고 동사를 URI에 넣지 않는다.

2. 주문 생성은 `POST /v1/orders`를 사용하고 성공 시 201과 Location을 반환한다.

3. 조회 성공은 200, 비동기 접수는 202, 본문 없는 삭제 성공은 204를 사용한다.

4. 검증 실패는 400, 인증 부재는 401, 권한 부족은 403, 충돌은 409로 구분한다.

5. 클라이언트는 주문 생성에 `Idempotency-Key`를 보내고 서버는 사용자와 키와 요청 해시를 저장한다.

6. 같은 키와 같은 요청은 기존 결과를 반환하고 같은 키와 다른 본문은 409로 거부한다.

7. 목록은 변경 중 중복과 누락을 줄이기 위해 정렬 키를 인코딩한 Cursor를 사용한다.

8. Bean Validation은 형식만 검사하고 재고와 가격 같은 업무 규칙은 서비스 계층에서 검증한다.

9. 오류 응답은 code, message, traceId, fieldErrors를 안정적인 계약으로 제공한다.

10. 호환 가능한 필드 추가는 같은 버전에서 수행하고 파괴적 변경만 새 주요 버전으로 분리한다.
...
public ResponseEntity<OrderResponse> create(
    ...
    OrderResponse result = orderService.create(key, request);
    return ResponseEntity.created(URI.create("/v1/orders/" + result.id())).body(result);
}
```
...
다음 Chapter에서는 [Data Architecture](/aws-backend/part-16/04-data-architecture/)를 학습한다.
