---
title: "Chapter 02. GitHub Actions"
permalink: /aws-backend/part-13/02-github-actions/
...
> **학습 목표**
>
> - GitHub Actions Workflow의 Trigger, Job, Step을 설명할 수 있다.
> - OIDC로 AWS Role을 Assume하고 장기 Access Key를 제거할 수 있다.
> - Gradle, BuildKit Cache, ECR Push를 하나의 Workflow로 구성할 수 있다.
---
# 수동 배포 실패 시나리오
장기 AWS Access Key를 GitHub Secret에 저장하면 유출 시 만료 전까지 재사용될 수 있고 저장소마다 Key를 회전해야 한다.
담당자의 기억과 로컬 상태에 의존하는 배포는 반복 가능하지 않고 감사 가능한 증거도 남기기 어렵다.
자동화의 목적은 명령을 빠르게 실행하는 데 그치지 않고 동일한 입력에 동일한 절차와 검증을 적용하는 데 있다.
---
# GitHub Actions란?
GitHub Actions는 저장소 이벤트로 Workflow를 실행하며 OIDC Token을 AWS STS에 제시해 짧은 수명의 Role Credential을 발급받을 수 있다.
...
# 전체 흐름
```text
GitHub Push -> Workflow Runner -> OIDC Token
                               |
                               v
                        AWS STS AssumeRole
                               |
                Gradle Test -> BuildKit -> ECR SHA Tag
```
각 단계는 입력 식별자와 출력 식별자를 기록하고 실패하면 다음 단계로 진행하지 않는다.
운영 배포 기록에는 Commit SHA, Image URI와 Digest, 실행 주체, 승인자, 시작과 종료 시각을 남긴다.
---
# 핵심 구성 비교
| 항목 | 장기 Access Key | OIDC Role |
|---|---|---|
| 보관 | GitHub Secret | 지속 Credential 없음 |
| 수명 | 회전 전까지 | 작업 단위 단기 |
| 통제 | Key Policy | Trust Policy와 Claims |
| 권장 | 권장하지 않음 | 권장 |
  ...
  image:
    ...
    steps:
      ...
      - uses: aws-actions/configure-aws-credentials@v4
        with:
          role-to-assume: arn:aws:iam::123456789012:role/github-order-api
      ...
      - uses: docker/build-push-action@v6
        with:
          context: .
          push: true
          tags: 123456789012.dkr.ecr.ap-northeast-2.amazonaws.com/order-api:${{ github.sha }}
          cache-from: type=gha
...
---
# 실무에서는 어떻게 사용할까
Workflow의 `permissions`는 `id-token: write`와 `contents: read`처럼 필요한 범위만 선언한다.

AWS Trust Policy의 `sub` Claim을 저장소, Branch 또는 GitHub Environment로 제한한다.

운영 Environment에 Reviewer 승인을 설정하면 승인 전에는 해당 Job의 Secret과 배포 단계가 실행되지 않는다.

배포 전에는 변경 위험도와 복구 가능성을 검토하고 배포 후에는 사용자 관점의 핵심 경로를 확인한다.
---
# 장애와 롤백
Fork Pull Request에는 운영 Role을 발급하지 않고 신뢰하지 않는 코드를 AWS Credential이 있는 Runner에서 실행하지 않는다.

BuildKit Cache는 빌드 시간을 줄이지만 최종 Image는 Commit SHA Tag와 Digest로 식별해야 한다.
...
---
# 다음 Chapter
다음 Chapter는 AWS 관리형 Build Runner인 CodeBuild와 buildspec의 실행 단계를 다룬다.
